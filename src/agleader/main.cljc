(ns agleader.main
  "Kotodama WASM entrypoint for the Agleader clean-room actor (L4) — Clojure port.
  ...
  facts (`agleader.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "agleader")
(def tier "L4")
...
(defn page-limit
  ...
  src/agleader/page_limit.kotoba."
  [requested]
  (if (pos? requested)
...
(defn emit-facts
  "EAVT facts for one record: {\"agleader.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "agleader-compat" :tier tier :entities entities} 200])
...
(defn start! [] :agleader-compat/ready)
