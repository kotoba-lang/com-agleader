(ns agleader.kotoba-paginate-qualification-test
  (:require [agleader.main :as oracle]
...
(def source-path "src/agleader/paginate.kotoba")
...
(deftest q9-paginate-kernel-oracle-and-backends-agree
  (let [source (slurp source-path)
        ...
        compiler-result (compiler-ir/execute (:kir compiler-artifact)
                                             'main [])]
    ...
    (is (true? (oracle/test-kernel)))
    (is (= #{} (get-in compiler-artifact [:hir :effects])))
    ;; Parity across the decision surface against the cljc oracle.
